...
The system SHALL provide a Pedido model with fields: id, usuario_id (FK), estado (FK to estado_pedido), total, direccion_id (nullable), forma_pago_id (nullable), mp_pago_id (nullable), creado_en, actualizado_en. The system SHALL enforce a finite state machine for estado transitions.
